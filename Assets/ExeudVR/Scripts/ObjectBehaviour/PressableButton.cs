...
namespace ExeudVR
{
    ...
    public class PressableButton : MonoBehaviour
    {
        ...
        private ReferenceFrame distanceReferenceFrame = ReferenceFrame.Local;

        /// <summary>
        /// Describes in which coordinate space the plane distances are stored and calculated
        /// </summary>
        public ReferenceFrame DistanceReferenceFrame
        {
            get => distanceReferenceFrame;
            set
            {
                // Convert world to local distances and vice versa whenever we switch the mode
                if (value != distanceReferenceFrame)
                {
                    distanceReferenceFrame = value;
                    float scale = (distanceReferenceFrame == ReferenceFrame.Local) ? WorldToLocalScale : LocalToWorldScale;
        ...
        private Dictionary<ControllerHand, Vector3> touchPoints = new Dictionary<ControllerHand, Vector3>();
        ...
        private Vector3 WorldSpacePressDirection
        {
            get
            {
                var nearInteractionTouchable = GetComponent<TouchableSurface>();
        ...
        {
            float distance = (distanceReferenceFrame == ReferenceFrame.Local) ? localDistance * LocalToWorldScale : localDistance;
